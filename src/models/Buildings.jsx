    ...
    <Html position={[0, 0, 0]} center distanceFactor={20} transform sprite>
      <div style={{
        ...
        <div style={{fontSize: '10px', fontWeight: 'bold', color: color, whiteSpace: 'nowrap'}}>
          <span style={{marginRight:'3px', opacity:0.8, fontSize:'9px'}}>Lv.{level}</span>
          {LABELS[type][lang]}
        </div>
        <div style={{
      ...
      <group position={[0, 0.05, 0]}>
        ...
        {level >= 3 && (
          <mesh position={[0, 2.8, 0]} castShadow>
            ...
            <meshStandardMaterial color={isMax ? "#e67e22" : "#95a5a6"} />
          </mesh>
        )}
        ...
        <group position={[0, level>=3 ? 3.5 : 3.2, 0]}>
...
export function GasStation({ position, lang = 'zh', owner, level=1 }) {
  ...
  return (
    <group position={position} scale={[scale, scale, scale]}>
      ...
      {/* Lv.3+ 增加便利店附属 */}
      {level >= 3 && <mesh position={[0, 1, -1]}><boxGeometry args={[2, 2, 1]} /><meshStandardMaterial color="white" /></mesh>}
...
export function TechOffice({ position, lang = 'zh', owner, level=1 }) {
  ...
  return (
    <group position={position} scale={[scale, scale, scale]}>
      {/* 满级变金色玻璃 */}
      <mesh position={[0, 2, 0]} castShadow><boxGeometry args={[2.5, 4, 2.5]} /><meshStandardMaterial color={isMax?"#f1c40f":"#3498db"} transparent opacity={0.8} /></mesh>
